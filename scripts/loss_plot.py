...
def read_log(log):
    ...
    with open(log, "r") as f:
        ...
        while line:
            ...
            # print(lineSp)
            if len(lineSp) == 6:
                if 'loss:' in lineSp and 'mae:' in lineSp:
                    try:
                        loss = float(lineSp[3])
                        mae = float(lineSp[5])
                        epoch = float(lineSp[1])
                        res.append([epoch, loss, 'Loss'])
                        res.append([epoch, mae, 'MAE'])
                    except Exception as e:
                        print(e)
                elif 'val_mae:' in lineSp and 'val_loss:' in lineSp:
                    try:
                        val_loss = float(lineSp[3])
                        val_mae = float(lineSp[5])
                        epoch = float(lineSp[1])
                        res.append([epoch, val_loss, 'Validation loss'])
                        res.append([epoch, val_mae, 'Validation MAE'])
                    except Exception as e:
                        print(e)
    df = pd.DataFrame(res, columns=['Epoch', 'Loss', 'Type'])
    return df
...
def line_plot(df, args):
    ...
    sns.lineplot(x='Epoch', y='Loss', data=ifg, hue='Type')
